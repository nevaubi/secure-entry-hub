...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Received standardized callback for:', payload.ticker, payload.status);
    ...
    // Update the most recent pending run for this ticker
    const updateResponse = await fetch(
      `${supabaseUrl}/rest/v1/standardized_processing_runs?ticker=eq.${payload.ticker}&status=eq.pending&order=created_at.desc&limit=1`,
      {
        method: 'PATCH',
        headers: {
          'apikey': supabaseKey,
    ...
    console.log(`Successfully updated standardized processing run for ${payload.ticker}`);
    ...
    console.error('Error in standardized-agent-callback:', error);
